...
def prepare_input(old_state, new_state):
    batch = np.zeros((2, INPUT_SIZE))
    batch[0, :] = build_input(env, old_state)
    batch[1, :] = build_input(env, new_state)
    return batch
...
if __name__ == '__main__':
    ...
    while True:
        ...
        while not end:
            ...
            network.train(prepare_input(old_state, new_state),
                          prepare_rewards([(max_action, my_reward)]))
            print_values(env.s)
        if my_reward == 10:
            total_wins += 1
            steps_to_win = 0
